...
int EventLoop::getVLANID(void)
{
    ...
    do
    {
        if (dbus_response.is_method_error())
        {
            log<level::ERR>("getVLANID: Error in Ipmi Server execute");
            break;
        }
                           ...
                           std::string channel, uint16_t reqPort)
{
    ...
    log<level::INFO>("Bind to interfae", entry("INTERFACE=%s", iface.c_str()));
